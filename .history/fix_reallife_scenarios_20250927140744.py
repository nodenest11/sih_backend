...
async def assess_safety_with_ai(tourist_id: int, latitude: float, longitude: float, location_id: int):
    """Simplified AI assessment function for testing"""
    try:
        # Basic safety score calculation
        base_score = 75
        
        # Check for restricted zones (simple coordinate check)
        restricted_areas = [
            (28.50, 77.08, "Remote area near Delhi"),  # Sample restricted area
            (15.25, 74.10, "Dangerous zone Goa")      # Another sample
        ]
        
        for r_lat, r_lon, r_name in restricted_areas:
            distance = abs(latitude - r_lat) + abs(longitude - r_lon)
            if distance < 0.1:  # Within ~10km
                base_score -= 30
                break
        
        # Speed-based adjustment (would need actual speed data)
        # For now, assume normal movement
        
        # Tourist-friendly zones boost
        tourist_areas = [
            (28.6129, 77.2295, "India Gate"),
            (15.2993, 74.1240, "Calangute Beach")
        ]
        
        for t_lat, t_lon, t_name in tourist_areas:
            distance = abs(latitude - t_lat) + abs(longitude - t_lon)
        ...
        assessment_data = {
            ...
            "severity": "SAFE" if safety_score > 70 else "WARNING" if safety_score > 40 else "CRITICAL",
            "confidence_level": 0.85,
            "geofence_alert": safety_score < 50,
            "anomaly_score": random.uniform(0.1, 0.3) if safety_score > 50 else random.uniform(0.7, 0.9),
            "temporal_risk_score": random.uniform(0.1, 0.4)
        }
        
        # Insert assessment
        supabase.table("ai_assessments").insert(assessment_data).execute()
        
        return assessment_data
        
    except Exception as e:
        print(f"AI assessment error: {str(e)}")
        return {"safety_score": 50, "severity": "WARNING", "confidence_level": 0.5}
...
async def test_fixed_reallife_scenarios():
    ...
    try:
        ...
        if registered_family:
            ...
            for tourist_id, (lat, lon, place) in zip(registered_family, locations):
                try:
                    location_data = {
                        "tourist_id": tourist_id,
                        "latitude": lat,
        ...
        try:
            result = supabase.table("tourists").insert(solo_tourist).execute()
            solo_id = result.data[0]["id"]
...
async def test_emergency_scenario_fixes():
    ...
    try:
        ...
        result = supabase.table("tourists").insert(emergency_tourist).execute()
        tourist_id = result.data[0]["id"]
        ...
        result = supabase.table("alerts").insert(sos_alert).execute()
