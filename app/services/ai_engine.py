...
class AIEngineService:
    ...
    def __init__(self):
        ...
        self.min_data_points = 25     # Minimum for training (reduced for faster iteration)
    ...
    async def check_and_retrain_models(self):
        ...
        try:
            ...
            for model_type in ['isolation_forest', 'temporal_autoencoder']:
                ...
                if (current_time - last_training).total_seconds() > self.retrain_interval:
                    logger.info(f"Retraining {model_type} model...")
                    
                    # Fetch fresh training data
                    df = await self.fetch_training_data(model_type)
                    ...
                    if not df.empty:
                        ...
                        logger.warning(f"No data available for {model_type} retraining")
    ...
    async def process_recent_locations(self):
        ...
        try:
            # Get locations from the last 5 minutes that haven't been processed
            ...
            ).limit(50).all()  # Process up to 50 locations at a time
            
            for location in recent_locations:
                await self.create_ai_assessment(location)
                
            if recent_locations:
                logger.info(f"Processed {len(recent_locations)} recent locations for AI assessment")
